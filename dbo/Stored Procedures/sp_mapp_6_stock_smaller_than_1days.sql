...
-- =============================================
CREATE PROCEDURE [dbo].[sp_mapp_6_stock_smaller_than_1days]
	@date date
 AS
...
SELECT S1.IBD_SEQ_ID,S1.IBD_PLANT,S1.IBD_SHOP,S1.IBD_SYSTEM,S1.IBD_MATERIAL
  ...
  AND S2.IDD_ONLINE_INVENTORY_AMT-S2.IDD_CLAC_AVG<S1.IBD_BOOKING_LIMIT
...
BEGIN
    -- 將資料插入到 target_table
	SET @MappMSg =　FORMAT(@date, 'yyyy-MM-dd')  +' '+@SHOP+@sysName+ '系統「' + @MATERIAL + '」化學品庫存<1日且未排入料請速確認排程。';

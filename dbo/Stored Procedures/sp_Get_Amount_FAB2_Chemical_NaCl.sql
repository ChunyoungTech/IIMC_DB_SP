...
CREATE PROCEDURE [dbo].[sp_Get_Amount_FAB2_Chemical_NaCl]
...
BEGIN
	...
	--M_SEQ_ID
	SELECT @M_SEQ_ID_W_UPWS = A.M_Seq_ID,@MAmount_UPWS=A.M_Amount,@M_FillingQty=A.M_FillingQty,@M_WeightTransferPara=A.M_WeightTransferPara  FROM  dbo.Material_Table  A 
	...
	/* 如有備註則終止計算 */
	IF EXISTS( SELECT M_Seq_ID FROM [Material_Table] WHERE [M_Seq_ID] IN(@M_SEQ_ID_W_UPWS) AND M_MenuRemark<>'')
	BEGIN 
	...
	--取得總用量
	EXEC sp_Get_FT_Amount @M_SEQ_ID_W_UPWS,'F2C10_UPWS_NaCl_001',1,0,@MAmount_T1 OUTPUT;
	EXEC sp_Get_FT_Amount @M_SEQ_ID_W_UPWS,'F2C10_UPWS_NaCl_001_D',2,3,@MAmount_T1A OUTPUT;
	EXEC sp_Get_FT_Amount @M_SEQ_ID_W_UPWS,'F2C10_UPWS_NaCl_001_COEF',5,3,@MAmount_T1C OUTPUT;

	--計算
	--sp_GetAmountPi 高,直徑,係數,比重,充填,輸出
	EXEC sp_GetAmountPi @MAmount_T1,@MAmount_T1A,@MAmount_T1C,@M_WeightTransferPara,@M_FillingQty,@MAmount_UPWS OUTPUT;

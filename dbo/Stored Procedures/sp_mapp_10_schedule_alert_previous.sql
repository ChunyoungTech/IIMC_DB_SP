...
-- Description:	<明日槽車入料通報>
--1.透過SDP_Order_Master 這張表查詢明天是否有安排出車
--2.JOIN [SDP_VW_SRM_CALLED_ORDERS_GCIC_ALLOCATE] 並將符合條件資料寫入[MappMessage]
-- =============================================
CREATE PROCEDURE [dbo].[sp_mapp_10_schedule_alert_previous]
	@date date
 AS
...
BEGIN
	...
	SELECT S1.SEQ_ID,S3.IBD_PLANT,S3.IBD_SHOP,S3.IBD_SYSTEM,S3.IBD_MATERIAL,S1.SOM_BOM,S2.SHIP_QTY,S2.UNIT,S2.VENDOR_ARRIVAL_DATE,S2.VENDOR_NAME
		...
		WHERE CONVERT(DATE,S2.VENDOR_ARRIVAL_DATE)=@date
	...
	BEGIN
		...
		set @msg= '明日槽車叫料 '+@IBD_SHOP+@IBD_SYSTEM+'系統['+@IBD_MATERIAL+'], 入料量'+ @SOM_ORDER_QTY +' ' + @SOM_ORDER_UNIT --+', 廠商' + @VENDOR_NAME

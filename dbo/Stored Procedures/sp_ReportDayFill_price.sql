...
-- =============================================
CREATE PROCEDURE [dbo].[sp_ReportDayFill_price]
...
FROM 
(
  SELECT M_Date,MB_Plant,MB_Shop,MB_Sysname,MB_CName,MB_UNIT,(M_FillingQty*M_Price) as M_FillingQty FROM Material_Table MT inner join Material_Base_Data MB ON MT.MB_SEQ_ID=MB.MB_SEQ_ID WHERE MB_IsVisable=''Y''
) AS j

...
CREATE PROCEDURE [dbo].[sp_Get_Amount_FAB8_Chemical_NaOCl]
	@CalcDate AS DATE --重新指定運算日期
AS
BEGIN
	
	SET NOCOUNT ON;
	--DECLARE @CalcDate AS DATE ='2024-2-25'
	...
	DECLARE @MB_Sysname varchar(50) ='WWTS';
        DECLARE @tagName1 varchar(50) ='F8W10W_WWTS_LIA_4707_PV';
        DECLARE @tagName1_D varchar(50) = @tagName1 + '_D';
        DECLARE @tagName1_COEF varchar(50) = @tagName1 + '_COEF';
        DECLARE @tagName2 varchar(50) ='F8W10W_WWTS_LIA_4707A_PV';
        DECLARE @tagName2_D varchar(50) = @tagName2 + '_D';
        DECLARE @tagName2_COEF varchar(50) = @tagName2 + '_COEF';
        ...
        DECLARE @MAmount_P1 decimal(18, 4)=0
        DECLARE @MAmount_P2 decimal(18, 4)=0
	...
	END
        ...
        --取得總用量
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W,@tagName1,1,0,@MAmount_T1 OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W,@tagName1_D,2,3,@MAmount_T1A OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W,@tagName1_COEF,5,3,@MAmount_T1C OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W,@tagName2,1,0,@MAmount_T2 OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W,@tagName2_D,2,3,@MAmount_T2A OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W,@tagName2_COEF,5,3,@MAmount_T2C OUTPUT;

        --計算
        --sp_GetAmountPi 高,直徑,係數,比重,充填,輸出
        EXEC sp_GetAmountPi @MAmount_T1,@MAmount_T1A,@MAmount_T1C,@M_WeightTransferPara,@M_FillingQty,@MAmount_P1 OUTPUT;
        EXEC sp_GetAmountPi @MAmount_T2,@MAmount_T2A,@MAmount_T2C,@M_WeightTransferPara,@M_FillingQty,@MAmount_P2 OUTPUT;
        SET @MAmount = ISNULL(@MAmount_P1,0)+ISNULL(@MAmount_P2,0);

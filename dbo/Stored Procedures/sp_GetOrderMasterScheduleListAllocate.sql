...
CREATE PROCEDURE [dbo].[sp_GetOrderMasterScheduleListAllocate]
				...
				left join (
					select (case
						...
						WHERE M.SOM_BOM like (CASE 
							WHEN @intBOM='ITO' THEN '34T%ITO%' 
							WHEN @intBOM='NMP' THEN '34T%NMP%'
							WHEN @intBOM='CSTR' THEN '34T%CSTR%'
							WHEN @intBOM='PGMEA' THEN '34T%PGMEA%'
							--WHEN @intBOM='ALL' THEN '34T___[^C]%'
							WHEN @intBOM='ALL' THEN '34T%'
							END)
							AND M.SOM_STATUS IN ('SENT','Closed','Wait_Cancel','Cancel')

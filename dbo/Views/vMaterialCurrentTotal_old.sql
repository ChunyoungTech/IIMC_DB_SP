...
CREATE VIEW [dbo].[vMaterialCurrentTotal_old]
AS
SELECT          M.M_Seq_ID, MB.MB_Seq_ID, MB.MB_Sysname AS Sysname, MB.MB_Plant AS Plant, MB.MB_Shop AS Shop, 
                            ...
                            M.M_TotalQty AS Amount, CONVERT(CHAR(10), DATEADD(DD, - 1, M.M_Date), 126) AS Yesterday, ISNULL
                                ((SELECT          M_TotalQty
...
WHERE          (MB.MB_IsVisable = 'Y') and MB.MB_IsMaster='Y'

...
CREATE PROCEDURE [dbo].[sp_Get_Amount_FABC_Chemical_NaAlO2]
...
BEGIN
	...
	DECLARE @CName varchar(50) ='鋁酸鈉';
	DECLARE @MB_Shop varchar(50) ='氣化課';
	DECLARE @MB_Sysname varchar(50) ='GSTS';


	DECLARE @MAmount decimal(18, 4)=0
	DECLARE @MAmount_F1 decimal(18, 4)=0
	DECLARE @MAmount_T1 decimal(18, 4)=0
	DECLARE @MAmount_T1A decimal(18, 4)=0
	DECLARE @MAmount_T1C decimal(18, 4)=0
	DECLARE @MAmount_T1D decimal(18, 4)=0
	...
	--取得總用量
	EXEC sp_Get_FT_Amount @M_SEQ_ID_W,'L2B1F_WWTS_T_712_LT',1,0,@MAmount_T1 OUTPUT;
	EXEC sp_Get_FT_Amount @M_SEQ_ID_W,'L2B1F_WWTS_T_712_LT_D',2,3,@MAmount_T1A OUTPUT;
	EXEC sp_Get_FT_Amount @M_SEQ_ID_W,'L2B1F_WWTS_T_712_LT_COEF',5,3,@MAmount_T1C OUTPUT;

	EXEC sp_Get_FT_Amount @M_SEQ_ID_W,'L2B1F_WWTS_T_712_LT_DF',4,3,@MAmount_T1D OUTPUT;


	--EXEC sp_Get_FT_Amount @M_SEQ_ID_W,'L2B1F_WWTS_NaOH_FT_ALL',1,1,@MAmount_F1 OUTPUT;

	--計算
	--sp_GetAmountPi 高,直徑,係數,比重,充填,輸出
	EXEC sp_GetAmountPi @MAmount_T1,@MAmount_T1A,@MAmount_T1C,@M_WeightTransferPara,0,@MAmount OUTPUT;

	set @MAmount=@MAmount/@MAmount_T1D

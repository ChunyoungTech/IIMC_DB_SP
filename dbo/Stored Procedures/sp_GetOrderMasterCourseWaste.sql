...
CREATE PROCEDURE [dbo].[sp_GetOrderMasterCourseWaste]
	@strSeqID as int		/*SDP_Order_Master_Waste流水號*/
AS
BEGIN
	...
	BEGIN TRY
		...
		select distinct 
			...
			,isnull(H.SHIP_QTY,'') as SHIP_QTY					--供應商確認可出貨數量
			...
			,isnull(H.VENDOR_REMARK,'') as REMARKS_Supplier		--備註說明(供應商回覆)
			--其他資訊--
			,SOM_STATUS --目前狀態(網頁上判斷狀態用)
			,D.STATUS as NowSTATUS --供應商目前狀態(網頁上判斷狀態用)
			--資料表來源--
			from SDP_Order_Master_Waste as M --廠務叫料單主檔
			inner join V_Inventory_BaseData as IBD on IBD_BOM_NO=M.SOM_BOM and IBD.IBD_PLANT=M.SOM_PLANT --庫存基本檔
			inner join V_Plant as P on P.Plant_T=SOM_PLANT --廠別代號對照
			left join (select * from SDP_VW_SRM_CALLED_ORDERS_GCIC union select * from SDP_VW_SRM_CALLED_ORDERS_GCIC_Waste) as D on D.CALLED_NO=M.SOM_SDP_NO --物管叫料單主檔
			left join Users as U on U.U_USER_ID=D.CREATE_BY --使用者
			--left join SDP_VW_ASN_DATA_GCIC as A on A.CALLED_NO=M.SOM_SDP_NO --AND A.VDR_SHIPPING_NO=M.SOM_SDP_NO --ASN 資料檔
			left join (select * from SDP_VW_SRM_CALLED_ORDERS_GCIC_HIS union select * from SDP_VW_SRM_CALLED_ORDERS_GCIC_HIS_Waste) as H on H.CALLED_NO=M.SOM_SDP_NO  --物管叫料單歷史

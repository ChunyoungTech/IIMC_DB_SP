...
-- Description:	<庫存預警 庫存< 2日 or <2日未確認排程>
-- =============================================
CREATE PROCEDURE [dbo].[sp_mapp_4_stock_smaller_than_2days]
	@date date
 AS
...
SELECT S1.IBD_SEQ_ID,S1.IBD_PLANT,S1.IBD_SHOP,S1.IBD_SYSTEM,S1.IBD_MATERIAL
  ...
  AND S2.IDD_ONLINE_INVENTORY_AMT-S2.IDD_CLAC_AVG*2<S1.IBD_BOOKING_LIMIT
...
BEGIN
    -- 將資料插入到 target_table
	SET @MappMSg =　FORMAT(@date, 'yyyy-MM-dd')  +' '+@SHOP+@sysName+ '系統「' + @MATERIAL + '」化學品庫存<2日且未排入料請速確認排程。';

	--SELECT @scheduledDate=VENDOR_ARRIVAL_DATE
		SELECT @scheduledDate=[ARRIVAL_DATE] --20250306改抓[ARRIVAL_DATE]避免廠商未確認一直發

CREATE PROCEDURE [dbo].[sp_Get_Amount_FAB8_Chemical_FeCl2]
        @CalcDate AS DATE --重新指定運算日期
AS
BEGIN

        SET NOCOUNT ON;
        --DECLARE @CalcDate AS DATE ='YYYY-MM-DD'
        DECLARE @StartDate AS DATE
        DECLARE @M_SEQ_ID_W_WWTS int
        DECLARE @Plant varchar(50) ='FAB8';
        DECLARE @CName varchar(50) ='FeCl2';
        DECLARE @MB_Shop varchar(50) ='水務課';
        DECLARE @MB_Sysname varchar(50) ='WWTS';
        DECLARE @tagName varchar(50) ='F8W10W_WWTS_LIA_4603_PV';
        ...
        --取得總用量
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W_WWTS,@tagName,1,0,@MAmount_T1 OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W_WWTS,@tagName_D,2,3,@MAmount_T1A OUTPUT;
        EXEC sp_Get_FT_Amount @M_SEQ_ID_W_WWTS,@tagName_COEF,5,3,@MAmount_T1C OUTPUT;

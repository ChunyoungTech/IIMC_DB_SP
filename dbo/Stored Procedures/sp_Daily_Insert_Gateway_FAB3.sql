...
-- 抓取Gateway
-- =============================================
CREATE PROCEDURE [dbo].[sp_Daily_Insert_Gateway_FAB3]
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		SELECT [UPDATETIME]
			...
			,[I_Tag]
			,[I_Value]
		FROM [C3C03247\SQLEXPRESS].[INX_IIMC].[dbo].[CIC_TABLE] WHERE UPLOAD_FLAG='N'

		UPDATE  [C3C03247\SQLEXPRESS].[INX_IIMC].[dbo].[CIC_TABLE] 
		SET UPLOAD_FLAG='Y'
		WHERE UPLOAD_FLAG='N'
	...
	BEGIN CATCH
		...
		Insert into DBO.Err_Message_Txt(Err_Object,Err_Flag,Err_Txt,Err_CRDate) VALUES('sp_Daily_Insert_Gateway_FAB3','Gateway抓取作業異常',@strError,getdate())
	END CATCH 

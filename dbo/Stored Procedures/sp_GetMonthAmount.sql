...
-- =============================================
CREATE PROCEDURE [dbo].[sp_GetMonthAmount]
...
FROM 
(
  select MMA.MB_SEQ_ID,MB_IsVisable,MB_Plant,MB_Shop,MB_Sysname,MB_CName,MB_UNIT,MMA_YM,MMA_Amount,MB.MB_DECIMAL_BITS from vMeterial_Month_Amount as MMA inner join Material_Base_Data as MB on MMA.MB_SEQ_ID = MB.MB_SEQ_ID
) AS j
...
ORDER BY MB_Plant,MB_Sysname,MB_CName,MB_Shop'
